export class ControlledRequest extends Request {
   ...
   * @param {*} request
   * @param {*} init
   * init.trafficPolicy object
   *   @param: retries: int,
   *   @param: attempt: int,
   *   @param: delay: int (time in ms)
   *   @param: abort: bool, // respond with 500
   *   @param: circuitBreaker: bool, // response with 503
   *   @param: timeout: int, // (time in ms)
   *   @param: loadbalance: [ { destination: '<ip>', weight: int }, ... ],
   *   @param: mirror: [ { destination: '<ip>', sample: int }, ... ]'
   */
